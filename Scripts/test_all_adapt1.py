...
class Testcase(unittest.TestCase):
    ...
    def test_all(self):
        ...
        self.driver.find_element_by_id("free.video.downloader.converter.music:id/tvContinue").click()
        #清空后 输入网站链接
        self.driver.find_element_by_id("free.video.downloader.converter.music:id/tvSearch").click()
        search_input = self.driver.find_element_by_id("free.video.downloader.converter.music:id/etInput")
        row = 0
        #批量链接
        for link in add_list:
            ...
            self.driver.find_element_by_id("free.video.downloader.converter.music:id/test_confirm_search_view").click()
            # time.sleep(8)
            ...
            #点击开始解析按钮
            download_bt = \
                self.driver.find_element_by_id("free.video.downloader.converter.music:id/normalView")
            download_bt.click()
            result = self.driver.find_element_by_id("free.video.downloader.converter.music:id/remindCountView")
            if  result :
                print ("解析成功 可正常下载")
            else:
                print("解析失败 或下载失败！ ")
